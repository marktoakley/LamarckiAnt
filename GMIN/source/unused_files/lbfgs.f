...
C>
C> \name LBFGS
C> \brief Limited memory BFGS method for large scale optimization ( J. Nocedal, July 1990 )
C>
C> 
...
C>      line search routine \b MCVSRCH, which is a slight modification of
C>      the routine \b CSRCH written by More and Thuente.
C> 
...
C> 
C> \b N       \n 
C>             is an INTEGER variable that must be set by the user to the
...
C> 
C> 
C> \b DIAG \n  is a DOUBLE PRECISION array of length N. If DIAGCO=.TRUE.,
...
C>             must be positive.
C> 
C> \b IPRINT \n is an INTEGER array of length two which must be set by the
C>             user.
C> 
...
C>             is less than XTOL.
C> 
C>     \b W \n      is a DOUBLE PRECISION array of length N(2M+1)+2M used as
C>             workspace for LBFGS. This array must not be altered by the
C>             user.
C> 
C> \b IFLAG \n is an INTEGER variable that must be set to 0 on initial entry
...
C> 
C> \b MP \n  is an INTEGER variable with default value 6. It is used as the
...
C> 
C> \b GTOL \n is a DOUBLE PRECISION variable with default value 0.9, which
...
C> 
C> \b STPMIN and \b STPMAX \n are non-negative DOUBLE PRECISION variables which
...
C>
! }}}
      SUBROUTINE LBFGS(N,M,X,F,G,DIAGCO,DIAG,IPRINT,EPS,XTOL,W,IFLAG,DGUESS,ITINC,BFGSTST,VEC,FTOL,INFO,AMBER)
! Declarations + help {{{
C
      INTEGER N,M,IPRINT(2),IFLAG
      DOUBLE PRECISION X(N),G(N),DIAG(N),W(N*(2*M+1)+2*M)
      DOUBLE PRECISION F,EPS,XTOL,DGUESS,DUMMY1,VEC(N)
      LOGICAL DIAGCO, BFGSTST, AMBER, MADELIST
      INTEGER qlistcount, rlistcount, listupdate
      INTEGER qlist(mxatms**2,2), rlist(mxatms**2,2)
      LOGICAL AMCUT
      DOUBLE PRECISION  QCUTOFF, RCUTOFF, REALQCUTOFF, REALRCUTOFF
      COMMON /LIST/ QCUTOFF,RCUTOFF,REALQCUTOFF,REALRCUTOFF,qlistcount,rlistcount,listupdate,AMCUT,qlist,rlist
...
C
     ...
     *                     GNORM,N,M,X,F,G,STP,FINISH,BFGSTST)
C
C    --------------------
...
C
         YS= DDOT1(N,W(IYPT+NPT+1),1,W(ISPT+NPT+1),1)
      IF(.NOT.DIAGCO) THEN
         YY= DDOT1(N,W(IYPT+NPT+1),1,W(IYPT+NPT+1),1)
         DO 90 I=1,N
   90    DIAG(I)= YS/YY
      ELSE
         IFLAG=2
...
C
 ...
 112  W(I)= -G(I)
      ...
      DO 125 I= 1,BOUND
         CP=CP-1
         IF (CP.EQ. -1)CP=M-1
         SQ= DDOT1(N,W(ISPT+CP*N+1),1,W,1)
         INMC=N+M+CP+1
         IYCN=IYPT+CP*N
         W(INMC)= W(N+CP+1)*SQ
         CALL DAXPY1(N,-W(INMC),W(IYCN+1),1,W,1)
 125  CONTINUE
C
      DO 130 I=1,N
 130  W(I)=DIAG(I)*W(I)
C
      DO 145 I=1,BOUND
         YR= DDOT1(N,W(IYPT+CP*N+1),1,W,1)
         BETA= W(N+CP+1)*YR
         INMC=N+M+CP+1
         BETA= W(INMC)-BETA
         ISCN=ISPT+CP*N
         CALL DAXPY1(N,BETA,W(ISCN+1),1,W,1)
         CP=CP+1
         IF (CP.EQ.M)CP=0
 145  CONTINUE
C
C  If this is a BFGSTS run project out the uphill direction.
C
      IF (BFGSTST) THEN
         DUMMY1=0.0D0
         DO J1=1,N
            DUMMY1=DUMMY1+VEC(J1)*W(J1)
         ENDDO
         DO J1=1,N
            W(J1)=W(J1)-VEC(J1)*DUMMY1
         ENDDO
      ENDIF
C
...
C     ----------------------------------------------------
 165  NFEV=0
      STP=ONE
C     STP=MAX(STP*1.1D0,ONE)  ! DJW
      IF (ITER.EQ.1) STP=STP1
      DO 170 I=1,N
 170  W(I)=G(I)
 172  CONTINUE
C     PRINT*,'Before MCSRCH INFO,IFLAG=',INFO,IFLAG
      CALL MCSRCH(N,X,F,G,W(ISPT+POINT*N+1),STP,FTOL,XTOL,MAXFEV,INFO,NFEV,DIAG)
C     PRINT*,'After MCSRCH INFO,IFLAG=',INFO,IFLAG
      IF (INFO .EQ. -1) THEN
        IFLAG=1
        RETURN
      ENDIF
...
C
C  This is when LBFGS thinks a step has been completed. INFO=1, but gets
C  set to -1 before the next step.
C
      IF (AMBER) THEN
      IF (MOD(ITER,listupdate).EQ.0 .AND. .NOT.(MADELIST) .AND. AMCUT) THEN
         QCUTOFF=REALQCUTOFF
         RCUTOFF=REALRCUTOFF
C         IF (RMS.GT.0.5D0) THEN
C            QCUTOFF=12.0D0
C            RCUTOFF=12.0D0
C         END IF
         CALL MAKELIST
         MADELIST=.TRUE.
         CALL POTENTIAL(X,G,F,.TRUE.,.FALSE.)
C         DO I=1,N
C            W(ISPT+POINT*N+I)=DGUESS*G(I)
C         ENDDO

C        WRITE(*,'(A)') 'Neighbour list updated'
      ELSE
         MADELIST=.FALSE.
      ENDIF
      ENDIF
...
C                        WRITE(MP,80) ITER,NFUN,F,GNORM,STP
                         WRITE(MP,81) F,GNORM/SQRT(1.0D0*N),ITER
C                        IF (.NOT.BFGSTST) CALL DUMPP(X)
                   ELSE
                         RETURN
                   ENDIF
              ELSE
...
C                  IF (.NOT.BFGSTST) CALL DUMPP(X)
              ENDIF
              IF (IPRINT(2).EQ.3.OR.IPRINT(2).EQ.3)THEN
                    IF (FINISH)THEN
                        WRITE(MP,90)
                    ELSE
                        WRITE(MP,40)
...
C
 ...
 80   FORMAT(2(I4,1X),3X,3(G20.10,2X))
 81   FORMAT(' Energy and RMS force=',2F20.10,' after ',I6,' LBFGS steps')
...
C> \name MCSRCH
C>                
C> \brief A slight modification of the subroutine CSRCH of More and Thuente.
C>
...
C>     where: \n
C>
C> \b N \n is a positive integer input variable set to the number
c>         of variables.
c>
C> \b X \n is an array of length N. On input it must contain the
c>         base point for the line search. on output it contains
c>         X + STP*S.
C>
C> \b F \n is a variable. On input it must contain the value of F
...
c>         of F at X + STP*S.
C>
C> \b S \n is an input array of length N which specifies the
c>         search direction.
c>
C> \b STP \n is a nonnegative variable. On input STP contains an
c>         initial estimate of a satisfactory step. On output
c>         stp contains the final estimate.
c>
C>  \b FTOL and \b GTOL \n are nonnegative input variables. (In this reverse
C>         communication implementation \b GTOL is defined in a \b COMMON
C>         statement.) Termination occurs when the sufficient decrease
c>         condition and the directional derivative condition are
c>         satisfied.
c>
C> \b XTOL \n is a nonnegative input variable. Termination occurs
c>         when the relative width of the interval of uncertainty
c>         is at most XTOL.
C>
C> \b STPMIN and \b STPMAX \n are nonnegative input variables which
c>         specify lower and upper bounds for the step. (In this reverse
C>         communication implementatin they are defined in a COMMON
C>         statement).
C>
C> \b MAXFEV \n is a positive integer input variable. Termination
c>         occurs when the number of calls to \b FCN is at least
c>         MAXFEV by the end of an iteration.
c>
C> \b INFO \n is an integer output variable set as follows:
c>
C>         INFO = 0  IMPROPER INPUT PARAMETERS.
C>
...
C
   ...
   10    CONTINUE
      IF (DGINIT .GE. ZERO) then
         write(LP,15)
   15    FORMAT(' In LBFGS the search direction does not appear to be a descent direction')
C
C   ... but actually, it probably is! DJW 16/5/00
C**********************
C        RETURN
C**********************
         ENDIF
...
C        Changed to allow DGINIT > 0. DJW 16/5/00
C
         IF (DGINIT .LT. ZERO) THEN
...
C  Changed to fix bug due to periodic boundary conditions. DJW 16/5/00
C  If atoms are put back into the primary box then the ANV values saved
C  to implement FIXIMAGE will be reset to X(J)+ 1.0 * S(J), but these
C  ANV values are then inappropriate for WA.
C
         DO 40 J = 1, N
C           X(J) = WA(J) + STP*S(J)
            X(J) = X(J) + (STP-PSTP)*S(J)
   40       CONTINUE
         INFO=-1
C        WRITE(*,'(A,3G20.10)') 'current  STP, F, DG=',STP, F, DG
C        WRITE(*,'(A,3G20.10)') 'best     STP, F, DG=',STX, FX, DGX
C        WRITE(*,'(A,3G20.10)') 'endpoint STP, F, DG=',STY, FY, DGY
C        PRINT*,'STP*S added to X, returning for F and G, STP=',STP,' INFO=-1'
         PSTP=STP
         RETURN
C
   45    INFO=0
...
C
C        WRITE(*,'(A,6G15.5)') 'STP,STPMIN,F,FTEST1,DG,DGTEST=',STP,STPMIN,F,FTEST1,DG,DGTEST
         IF ((BRACKT .AND. (STP .LE. STMIN .OR. STP .GE. STMAX)) .OR. INFOC .EQ. 0) THEN
            INFO = 6
            WRITE(*,'(A,I1)') 'In MCSRCH, rounding errors prevent further progress, INFO=',INFO
         ELSE IF (STP .EQ. STPMAX .AND. F .LE. FTEST1 .AND. DG .LE. DGTEST) THEN
            INFO = 5
            WRITE(*,'(A,I1)') 'In MCSRCH, the step is at the upper bound stpmax, INFO=',INFO
         ELSE IF (STP .EQ. STPMIN .AND. (F .GT. FTEST1 .OR. DG .GE. DGTEST)) THEN
            INFO = 4
            WRITE(*,'(A,I1)') 'In MCSRCH, the step is at the lower bound stpmin, INFO=',INFO
         ELSE IF (NFEV .GE. MAXFEV) THEN
            INFO = 3
            WRITE(*,'(A,I1)') 'In MCSRCH, number of function calls has reached maxfev, INFO=',INFO
         ELSE IF (BRACKT .AND. STMAX-STMIN .LE. XTOL*STMAX) THEN
            INFO = 2
            WRITE(*,'(A,I1)') 'In MCSRCH, relative width of the interval of uncertainty < xtol, INFO=',INFO
C
C  Changed to allow rapid escape from the case where DGINIT.GT.ZERO, DJW 16/5/00
C
         ELSE IF (F .LE. FTEST1 .AND. ((ABS(DG) .LE. GTOL*(-DGINIT)).OR.(DGINIT.GT.ZERO))) THEN
C        ELSE IF (F .LE. FTEST1 .AND. ABS(DG) .LE. GTOL*(-DGINIT)) THEN
            INFO = 1
         ENDIF
C
...
C        WRITE(*,'(A,L,4G20.10)') 'STAGE1,F,FTEST1,DG,DGINIT=',STAGE1,F,FTEST1,DG,DGINIT
...
C
            ...
            DGM = DG - DGTEST
C
C  Setting FTOL to 1 makes DGX and DGTEST equal and constitutes an error.
C
            DGXM = DGX - DGTEST
            DGYM = DGY - DGTEST
C
...
C           PRINT*,'After MCSTEP branch 1 STP=',STP
C
...
C           STPSAVE=STP
     ...
     *                 BRACKT,STMIN,STMAX,INFOC)
C           STP=STPSAVE/1.1D0
C           PRINT*,'After MCSTEP branch 2 STP=',STP
            END IF
C
...
C>     where
C>
C> \b STX, \b FX, and \b DX \n are variables which specify the step,
...
C>
C> \b BRACKT \n is a logical variable which specifies if a minimizer
...
C>
C> \b STPMIN and \b STPMAX \n are input variables which specify lower
...
C
      END
C
      subroutine daxpy1(n,da,dx,incx,dy,incy)
c
