...
def exchange_atoms_ring1_tyr(a, aa, residue, dihedrals):
# n - different orders of atoms in IMPROPER (the third atom is always fixed)
# 0: 0-1-3
# 1: 0-3-1
# 2: 1-0-3
...
# 5: 3-1-0
  ...
  find_atom1 = a[aa.index(residue)].index('CB')
  atomNumber1 = find_atom1+currentAtomNumber
  atomNumberIndex1 = atomNumber1*3
  find_atom2 = a[aa.index(residue)].index('CD1')
  atomNumber2 = find_atom2+currentAtomNumber
  atomNumberIndex2 = atomNumber2*3
  find_atom3 = a[aa.index(residue)].index('CG')
  atomNumber3 = find_atom3+currentAtomNumber
  ...
  n5 = 0
  for j in range(len(dihedrals)):
    if ((dihedrals[j][0]==str(atomNumberIndex1)) and (dihedrals[j][1]==str(atomNumberIndex2)) and (dihedrals[j][2]=="-"+str(atomNumberIndex3)) and (dihedrals[j][3]=="-"+str(atomNumberIndex4))):
#      print "n5 = 4"
      n5 = 4
    elif ((dihedrals[j][0]==str(atomNumberIndex2)) and (dihedrals[j][1]==str(atomNumberIndex1)) and (dihedrals[j][2]=="-"+str(atomNumberIndex3)) and (dihedrals[j][3]=="-"+str(atomNumberIndex4))):
#      print "n5 = 5"
...
#      print "n5 = 3"
      n5 = 3
    elif ((dihedrals[j][0]==str(atomNumberIndex4)) and (dihedrals[j][1]==str(atomNumberIndex1)) and (dihedrals[j][2]=="-"+str(atomNumberIndex3)) and (dihedrals[j][3]=="-"+str(atomNumberIndex2))):
#      print "n5 = 0"
      n5 = 0
    else: continue
...
######################################
## reading uni_aminont03.lib library
######################################

print '  uni_aminont03.lib'
aalib = open("%s/uni_aminont03.lib" % path).read()
aa = string.split(aalib, "\n")
q1 = aa.index("!!index array str")
q2 = aa.index("!entry.NALA.unit.atoms table  str name  str type  int typex  int resx  int flags  int seq  int elmnt  dbl chg")
...
######################################
## reading uni_aminoct03.lib library
######################################

print '  uni_aminoct03.lib'
aalib = open("%s/uni_aminoct03.lib" % path).read()
aa = string.split(aalib, "\n")
q1 = aa.index("!!index array str")
...
for i in range(len(residueLabel)):
  if ((aaNames.count(residueLabel[i])==0) and (aactNames.count(residueLabel[i])==0) and (aantNames.count(residueLabel[i])==0) and (group7.count(residueLabel[i])==0)):
...
#  print '----', i+1, '----', residueLabel[i]
  if (group7.count(residueLabel[i])>0): continue
    ...
    if (group0n.count(residueLabel[i])>0): 
      ...
      exchange_atoms_nglu('OE1', antTypes, aantNames, residueLabel[i], dihedralsWithoutHydrogen,2)
    elif (residueLabel[i]=='NPHE'):
      exchange_atoms_ring1_phe(antTypes, aantNames, residueLabel[i], dihedralsWithoutHydrogen)
      ...
      exchange_atoms_ring1_tyr(antTypes, aantNames, residueLabel[i], dihedralsWithoutHydrogen)
      exchange_atoms_ring2(antTypes, aantNames, residueLabel[i], dihedralsIncHydrogen)
...
#          print '!!!', dihedralsWithoutHydrogen[j], atomNumberIndex1, dihedralsWithoutHydrogen[j].index(str(atomNumberIndex1))
    ...
    if (group1.count(residueLabel[i])>0): ## res belongs to group1
      ...
      exchange_atoms_ring1_phe(actTypes, aactNames, residueLabel[i], dihedralsWithoutHydrogen)
      ...
      exchange_atoms_ring1_tyr(actTypes, aactNames, residueLabel[i], dihedralsWithoutHydrogen)
      exchange_atoms_ring2(actTypes, aactNames, residueLabel[i], dihedralsIncHydrogen)
      exchange_atoms('O', actTypes, aactNames, residueLabel[i], dihedralsWithoutHydrogen, currentAtomNumber)
...
#    elif (residueLabel[i]=='ASP'):
#      exchange_atoms_asp('OD1', aTypes, aaNames, residueLabel[i], dihedralsWithoutHydrogen, currentAtomNumber)
      ...
      exchange_atoms_ring1_phe(aTypes, aaNames, residueLabel[i], dihedralsWithoutHydrogen)
      ...
      exchange_atoms_ring1_tyr(aTypes, aaNames, residueLabel[i], dihedralsWithoutHydrogen)
      exchange_atoms_ring2(aTypes, aaNames, residueLabel[i], dihedralsIncHydrogen)
