...
class Atom(object):
    ...
    def __init__(self, index, name, mass, amber_atom_type, charge, molecule, *args, **kwargs):
        self.index = index
    ...
    def __cmp__(self, other):
        ...
        if self.mass < other.mass:
            ...
            raise ex.ValueError("Atom comparison didn't work.")
        return cmp_status
            ...
            for residue in molecule.residues.nodes():
                if residue.name == param[0]:
                    ...
                    output.write(" ".join(("GROUP", 
                                           ...
                                           str(params[param][1]),
                                           "\n")))
                    for atom in sorted(rotated_atoms[2], key=lambda x: x.index):
...
def get_rotated_atoms(bond):
    ...
    rotating_atoms = sorted(sorted(list(nx.connected_components(residue.molecule.atoms)), key=len)[0])
...
if __name__ == "__main__":
    ...
    parsed = create_atoms_and_residues(topology_data)
    group_rot_file = group_rotation_file(parsed, amino.def_parameters, sys.argv[2])
